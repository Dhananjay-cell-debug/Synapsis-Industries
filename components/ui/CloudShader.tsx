...
void main(void) {
	...
	for (float i=1.; i<12.; i++) {
		...
		vec2 p=uv;
		float d=length(p);
		col+=.00125/d*(cos(sin(i)*vec3(1.0, 0.478, 0.604))+1.);
		float b=noise(i+p+bg*1.731);
		col+=.002*b/length(max(p,vec2(b*p.x*.02,p.y)));
		col=mix(col,vec3(bg*.75,bg*.85,bg*.70) * 0.3,d);
	}
...
// Extracted just the background to use it in any layout
export default function CloudShader({ className = "" }: { className?: string }) {
    const canvasRef = useShaderBackground();
    return (
        <canvas
            ref={canvasRef}
            className={`absolute inset-0 w-full h-full object-cover touch-none ${className}`}
            style={{ background: '#b5c0a3', zIndex: 0 }}
        />
    );
}
